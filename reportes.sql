...
SELECT p.TRACKING,
       ...
       GET_TRAVEL_STEP(U.LATITUD, U.LONGITUD, U2.LATITUD, U2.LONGITUD
           , TRUNC(MOD((p.FECHAS.FECHA_FIN-p.FECHAS.FECHA_INICIO) * (60*24),60)), 'Lat')
FROM PEDIDO p
INNER JOIN DIRECCION D on D.ID = p.ID_DIRECCION and D.CED_CLIENTE = p.ID_CLIENTE_DIRECCION and D.ID_ZONA = p.ID_ZONA_DIRECCION
...
GROUP BY p.TRACKING, p.FECHAS.FECHA_INICIO, p.FECHAS.FECHA_FIN, C2.EMAIL, GET_TRAVEL_STEP(U.LATITUD, U.LONGITUD, U2.LATITUD, U2.LONGITUD
           , TRUNC(MOD((p.FECHAS.FECHA_FIN-p.FECHAS.FECHA_INICIO) * (60*24),60)), 'Lat')
ORDER BY p.TRACKING;
